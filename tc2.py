...
# Load data
data = pd.read_csv('/Users/apple/adjusted_data2.csv')
...
def create_model(input_shape):
    model = tf.keras.models.Sequential([
        tf.keras.layers.Input(shape=(input_shape,)),  # Ensure this matches x_train.shape[1]
...
# Check model summary before starting training
model = create_model(x_train.shape[1])
print(model.summary())  # Print the model summary to check the input/output shapes
